...
public class StatePlayersInstantiation implements State {
	...
	public void process(StateContext context, ControladorDeJogo controladorDeJogo) {
		...
		for (int i = 0; i < numeroDeJogadoresIA; i++) {
			// percorre circularmente a lista de estratégias
			int estrategiasListIndex = i % numeroDeEstrategias;

			// identificador da estratégia utilizada
			String estrategia = estrategias.get(estrategiasListIndex);

			Jogador novoJogadorIA = fabricaDeJogador.getJogadorIA(Integer.toString(i), estrategia);
